...
codePtr = ensureDataType("code", typeManager, 1, None)
for i in range(len(metaDataTypes)):
    ...
    if name in knownVtable.keys():
        ...
        for ref in accu:
            ...
            if code.split(": ")[1].startswith("this->vtable = ") and "&PTR_~{}_".format(name) in code:
                extractedAddr = getAddress(code.split("_")[-1].replace(";", ""))
    ...
    for funcName in vtable:
        if funcName.startswith("field_0x"):
            vtableStruct.add(codePtr, 8, funcName, "Generated by RedMetaClassAnalyzer.py")
        else:
    ...
    dataType.replace(0, vtableStructPtr, 8, "vtable", "Generated by RedMetaClassAnalyzer.py")
